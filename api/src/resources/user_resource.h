...
protected:
    ...
    Ptr<User> createObject(const Wt::Json::Object& json, Session& session) const override;
    void sendUpdatedInfo(const Ptr<User>& user) const override;
    void sendDeletedInfo(const Ptr<User>& user) const override;
};
